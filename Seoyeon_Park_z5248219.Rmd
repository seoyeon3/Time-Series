...
In this question, you are going to continue analysing **Question 1** in
**Assignment 1**. You should analyse the **same country** as you did in
**Assignment 1**:
...
Take the steps mentioned below and **explain the output of EACH STEP in
brief**:

Load the data for your assigned country and call it $X_t$. Note that you
should **NOT** apply any transformation or diff to the original data in
this assignment.
...
**1.a** What are the differences between the two fitted model based on
the orders of $p,\ d,\ q,\ P,\ D,\ Q$ and $S$?
...
**1.b** Compare Model 1 and Model 2 based on the AIC and BIC. Which
...
**1.c** Do the diagnostic checking for the residuals. DO the plots
confirm your choice? Mention at lease **two points**.
...
**1.d** Do the forecast for the next 12 months using ${\tt sarima.for}$.
Which model results in a better forecast? why?
...
**1.e** Extract the **residuals** of your preferred mode. Plot the
**periodogram** of the residuals and specify the **maximum value(s)**
along with the corresponding **frequency(ies)**.
...
**For the following question, we are going to smooth the original time
series assigned to you in Question 1 using different methods**. So, in
**each part**, you need to load the data related to your assigned
country and then apply the specified method of smoothing.

**2.a** Use **symmetric moving average** filters with ${\tt k}=2$ and
${\tt k}=6$ and plot the time series along with the fitted smootherss.
Determine which value of ${\tt k}$ provides a smoother trend and which
one captures the seasonal behavior of the time series?
...
**2.b** Use **Kernel Smoothing** with **normal** kernel with
${\tt bandwidth}=0.1, 0.5, 1, 2$. Plot the time series along with the
fitted smoothers. Determine which ${\tt bandwidth}$ value provides a
better smoother for the trend and which one better captures the seasonal
...
**2.c** Use **Lowess** with smoother span of ${\tt f}=0.01, 0.5, 0.9$.
Plot the time series along with the fitted smoothers. Determine which
${\tt f}$ provides a better smoother for the trend and which one better
captures the seasonal behavior of the time series.
...
**2.d** Use **Smoothing Splines** with smoothing parameter
${\tt spar}=0.01, 0.5, 0.9$, along with using ${\tt cv=TRUE}$ instead of
specifying the value of ${\tt spar}$. Plot the time series along with
the fitted smoothers. Determine which ${\tt spar}$ value provides a
better smoother for the trend and which one better captures the seasonal
behavior of the time series.
...
**3.a** Find the autocovariance function of $X_t$ and use it to show $$
f_X(\omega) = [1 + A^2 + 2A\cos(2\pi \omega D)] f_Y(\omega) + f_V(\omega).
$$

-   refer to q3a.pdf

**3.b** Conduct the following simulation study to verify the result in
**3.a**. Use the 4 last digits of your student ID as ${\tt set.seed}$.
For example, if your student ID is 1234567 then use
${\tt set.seed(4567)}$ and if it is 1230567, use ${\tt set.seed(567)}$.
